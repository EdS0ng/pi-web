...
- Desktop gotcha: URL-restored `?view=` does not open the workspace panel, and its default collapse state has changed across upstream releases (it used to start collapsed on desktop; it no longer does). Never assume — use `openWorkspaceTool(page, "Files" | "Git" | "Terminal")` from `e2e/support/fixtures.ts`, which expands only when the edge control offers "Expand workspace panel" and then clicks the tool tab. (On mobile viewports use the `app-mobile-main-tabs` buttons instead.)
