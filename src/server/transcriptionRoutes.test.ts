...
import { createCodexAuthProvider, extractCodexAccountId, registerTranscriptionRoutes, type CodexAuthProvider } from "./transcriptionRoutes.js";
...
describe("createCodexAuthProvider", () => {
  const dirs: string[] = [];

  async function agentDirWithAuth(auth: unknown): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), "pi-web-codex-auth-"));
    dirs.push(dir);
  ...
  it("reads the OAuth access token from the agent profile's auth.json", async () => {
    const dir = await agentDirWithAuth({ "openai-codex": { type: "oauth", access: VALID_TOKEN, refresh: "r", expires: 1 } });
    const provider = createCodexAuthProvider({ agentDir: () => Promise.resolve(dir) });

    await expect(provider.getAccessToken()).resolves.toBe(VALID_TOKEN);
  });

  // The web process is long-lived, but the session daemon refreshes the token on
  // disk; a cached read would keep serving an expired one.
  it("re-reads the file on every call so a refreshed token is picked up", async () => {
    const dir = await agentDirWithAuth({ "openai-codex": { type: "oauth", access: "first", refresh: "r", expires: 1 } });
    const provider = createCodexAuthProvider({ agentDir: () => Promise.resolve(dir) });
    await expect(provider.getAccessToken()).resolves.toBe("first");

    await writeFile(join(dir, "auth.json"), `${JSON.stringify({ "openai-codex": { type: "oauth", access: "second", refresh: "r", expires: 2 } })}\n`, "utf8");

    await expect(provider.getAccessToken()).resolves.toBe("second");
  });

  it("returns undefined when the provider is absent or has no usable token", async () => {
    const missing = await agentDirWithAuth({ anthropic: { type: "oauth", access: "other", refresh: "r", expires: 1 } });
    await expect(createCodexAuthProvider({ agentDir: () => Promise.resolve(missing) }).getAccessToken()).resolves.toBeUndefined();

    const empty = await agentDirWithAuth({ "openai-codex": { type: "api_key" } });
    await expect(createCodexAuthProvider({ agentDir: () => Promise.resolve(empty) }).getAccessToken()).resolves.toBeUndefined();
  });
});
